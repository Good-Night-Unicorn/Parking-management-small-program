<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="进入时间" prop="jinrushijian">
            <el-date-picker
        ...
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.jinrushijian" label="进入时间" prop="jinrushijian">
              <el-input v-model="ruleForm.jinrushijian" 
                placeholder="进入时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  computed: {
    zongjia:{
      get: function () {
        return 1*this.ruleForm.tingcheshizhang*this.ruleForm.xiaoshidanjia
      }
    },
  },
  created() {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `tingchefeiyong/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.tingchefeiyongCrossAddOrUpdateFlag = false;
                  this.parent.search();
