<template>
	<view class="content">
		<form class="app-update-pv">
				...
				<view :style='{"width":"160rpx","fontSize":"28rpx","color":"rgba(21, 20, 20, 1)","textAlign":"left"}' class="title">停放时间</view>
				<picker mode="date" :value="ruleForm.tingfangshijian" @change="tingfangshijianChange">
					<view :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0.56)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(20, 19, 19, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' class="uni-input">{{ruleForm.tingfangshijian?ruleForm.tingfangshijian:"请选择停放时间"}}</view>
				</picker>
			</view>
...
<script>
	...
	export default {
		...
		async onLoad(options) {
    		        this.ruleForm.tingfangshijian = this.$utils.getCurDate();
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`cheliangtingfang`, this.ruleForm.id);
			...
			if(options.cross){
				...
				for (var o in obj){
					...
					if(o=='tingfangshijian'){
					this.ruleForm.tingfangshijian = obj[o];
					this.ro.tingfangshijian = true;
					continue;
					}
					if(o=='beizhu'){
		...
		methods: {
			...
			// 多级联动参数
																											...
																											tingfangshijianChange(e) {
				this.ruleForm.tingfangshijian = e.target.value;
				this.$forceUpdate();
			},
			...
			async onSubmitTap() {
					...
					return
																																																																																																																																				...
																																																																																																																																				if(this.ruleForm.id){
					await this.$api.update(`cheliangtingfang`, this.ruleForm);
				}else{
					await this.$api.add(`cheliangtingfang`, this.ruleForm);
				}
